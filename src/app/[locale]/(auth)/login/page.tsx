...
function LoginContent() {
  ...
  const handleLinkedInLogin = async () => {
    await supabase.auth.signInWithOAuth({
      ...
      options: {
        redirectTo: `${window.location.origin}/auth/callback?redirect=${redirect}&locale=${locale}`,
      },
    });
  };
    ...
    <main className="min-h-screen bg-white flex items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm">
        ...
        <p className="mt-8 text-center text-sm text-[#37352f]/60">
          {t('noAccount')}{' '}
          <Link href={`/${locale}/signup`} className="text-[#0077cc] hover:underline font-medium">
            {t('signUp')}
          </Link>
        </p>
      </div>
